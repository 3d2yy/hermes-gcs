    ...
    def __init__(self, i2c_bus):
        ...
        try:
            if MPU6050_AVAILABLE:
                devices = self.i2c.scan()
                # Auto-detection of address (0x68 or 0x69)
                mpu_addr = 0x68 if 0x68 in devices else (0x69 if 0x69 in devices else None)
                
                if mpu_addr:
                    self.mpu = MPU6050(bus=self.i2c, addr=mpu_addr)
                    self.available = True
                    print(f" MPU6050 inicializado en {hex(mpu_addr)}")
                else:
                    print(" MPU6050 no detectado (ni 0x68 ni 0x69)")
            else:
    ...
    def __init__(self, i2c_bus):
        ...
        for pcf_num, addr in PCF8574_ADDRESSES.items():
            if addr in devices:
                ...
                print(f" [WARN] PCF8574 #{pcf_num} no encontrado")
...
class ESP32UnifiedSystem:
    def __init__(self):
        ...
        try:
            # I2C Scan as diagnostics
            self.i2c = I2C(0, scl=Pin(I2C_SCL_PIN), sda=Pin(I2C_SDA_PIN), freq=I2C_FREQUENCY)
            print("[I2C] Scanning devices...")
            devices = self.i2c.scan()
            print(f"[I2C] Found devices: {[hex(x) for x in devices]}")
        except Exception as e:
            print(f" Error I2C: {e}")
            raise
